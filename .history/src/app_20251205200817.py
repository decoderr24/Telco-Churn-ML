...
def load_model():
    try:
        # Memuat model yang sudah dituning
        model = joblib.load('churn_prediction_model.pkl')
        return model
    except FileNotFoundError:
        st.error("File 'churn_prediction_model.pkl' tidak ditemukan. Pastikan file ada di folder yang sama.")
        return None
...
if st.button('🚀 Prediksi Churn'):
    if model:
        ...
        # LOGIKA THRESHOLD (Hasil Tuning Anda)
        THRESHOLD = 0.4426
        ...
        if prediction_proba > THRESHOLD:
            ...
            st.write(f"Probabilitas Churn: {prediction_proba:.2%}")import streamlit as st
import pandas as pd
import joblib
